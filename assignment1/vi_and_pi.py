	...
	while True:
		...
		for s in range(nS):
			...
			for probability, nextstate, reward, terminal in P[s][a]:
				newVal += probability * (reward + gamma * value_function[nextstate])
			value_function[s] = newVal
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for s in range(nS):
		...
		for a in range(nA):
			for probability, nextstate, reward, terminal in P[s][a]:
				A[a] += probability * (reward + gamma * value_from_policy[nextstate])
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for s in range(nS):
			...
			for a in range(nA):
				for probability, nextstate, reward, terminal in P[s][a]:
					A[a] += probability * (reward + gamma * value_function[nextstate])
			value_function[s] = max(A)
			delta = max(delta, np.abs(v - value_function[s]))
		if delta < tol:
			break

	# print("action values:", A)
...
if __name__ == "__main__":
	...
	"""
	p = np.zeros(env.nS)
	v = policy_evaluation(env.P, env.nS, env.nA, p)
	print(v)
	new_p = policy_improvement(env.P, env.nS, env.nA, v, p)
	print(new_p)
	v = policy_evaluation(env.P, env.nS, env.nA, new_p)
	print(v)
	new_p = policy_improvement(env.P, env.nS, env.nA, v, p)
	print(new_p)
	
	v, p = policy_iteration(env.P, env.nS, env.nA)
	print(p)
	print(v)
	
	v, p = value_iteration(env.P, env.nS, env.nA)
	print(v)
	print(p)
	"""
